...
public class FlipViewController extends AdapterView<Adapter> {
  ...
  public FlipViewController(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray
        a =
        context.getTheme().obtainStyledAttributes(attrs, com.sender2.ui.R.styleable.FlipViewController, 0, 0);

    try {
      int value = a.getInteger(com.sender2.ui.R.styleable.FlipViewController_orientation, VERTICAL);
      if (value == HORIZONTAL) {
        orientation = HORIZONTAL;
      }

      value = a.getInteger(com.sender2.ui.R.styleable.FlipViewController_animationBitmapFormat, 0);
      if (value == 1) {
